...
2. display:none 和 visibility:hidden 的区别？
   ...
   display:none 隐藏对应的元素，在文档布局中不再给它分配空间，它各边的元素按照排列书序占据它的位置，但是会在 dom 树上存在。
...
19. 对媒体查询的理解？
    ...
    判断元素在不同屏幕大小时，以及横竖状态需要展示的状态，在 link 标签中使用 媒体查询，在某些屏幕大小下加载指定 css 文件。

20. CSS 优化、提高性能的方法有哪些？

    避免过度约束、避免后代选择符、避免链式选择符、避免使用 \* 选择器、避免不必要的命名空间、避免不必要的重复、最好使用表示语义的名字。一个好的类名应该是描述、避免 `!important`，可以选择其他选择器、尽可能的精简规则，你可以合并不同类里的重复规则
...
34. 伪类和伪元素区别？
    ...
    伪类，就像 :hover :visite :active 等，在某些鼠标行为下激活的样式，不会添加新的元素，只会根据行为作用在特定元素上。

    伪元素:after :before 这种，在元素子元素前后增加元素，默认行内元素。

35. position:fixed;在 android 下无效怎么处理？
...
36. 如果需要手动写动画，你认为最小时间间隔是多久，为什么？

    和屏幕刷新率有关，多数显示器默认频率是 60Hz，即 1 秒刷新 60 次，所以理论上最小间隔为 1/60＊1000ms ＝ 16.7ms

37. li 与 li 之间有看不见的空白间隔是什么原因引起的？有什么解决办法？
...
41. CSS 预处理和后处理器是什么？
    ...
    less、sass、stylus 等，扩展 css 能力，使 css 具有可编程能力，可以封装函数，mixin 混入变量、循环等等。

42. CSS 如何开启硬件加速

    当使用 CSS3 中新增的 transoform，will-change 等属性，浏览器会使用 GPU 去渲染。

    需要注意层爆炸的问题，使用 z-index 来规避。
