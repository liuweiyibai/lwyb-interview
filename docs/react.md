...
- 类组件和函数式组件有什么区别？
  ...
  首先在被 Bable 编译出来结果也不一样。Hooks 状态变化的话会重新执行组件，类组件会重新执行几个生命周期函数。

  类组件基于 this 做各种状态化操作。类组件并不是 真正的实现 ui = fn(state)
...
- 在 React 中我们怎么做静态类型检测？都有哪些方法可以做到？
  ...
  根据官方提供的 propType 库来做

- 在 React 中组件的 state 和 setState 有什么区别？

  组件 state 是组件内部状态，通过 setState 触发组件更新

- React 怎样跳过重新渲染？

  shouldComponentUpdate 根据返回值布尔值决定组件是否更新
